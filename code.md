제시해주신 모든 규칙과 아키텍처 가이드를 통합하여, LLM에게 입력하기 가장 좋은 **마크다운(Markdown) 형식의 시스템 프롬프트**로 정리해 드립니다.
이 내용을 복사해서 사용하시면 LLM이 정의된 구조와 규칙을 엄격히 준수하며 코드를 생성하게 됩니다.
# 🚀 FastAPI 클린 아키텍처 시스템 프롬프트
```markdown
